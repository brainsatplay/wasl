...
${text2}`;
              ...
              if (!language || js.includes(language)) {
                if (node.src.text) {
                  const esmImport = async (text2) => {
                    try {
                      let imported = await importFromText(text2);
  ...
  function cursors_default(context) {
  ...
  function player_default(context) {
    console.warn("Should Resolve", this.bounce, this.collideWorldBounds);
  ...
  function createPlayer(player) {
    console.log("Create player", player, this);
  ...
  var options = {
    ...
    filesystem: {
      ...
      "scripts/player/create.js": create_exports2,
      "scripts/player/update.js": update_exports
    }
  };
