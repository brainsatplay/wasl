...
${text}`;
  ...
  var ESPlugin = class {
    ...
    constructor(node, options2 = {}) {
      ...
      let lastRendered;
      const render = () => {
        if (lastRendered != this.element && typeof this.onrender === "function") {
          this.onrender();
          lastRendered = this.element;
        }
      };
      Object.defineProperty(this, "parentNode", {
        ...
        set: (el) => {
          ...
          if (el) {
            if (this.element) {
              console.log("RENDERING FOR PARENT");
        ...
        const runIfMatch = async (target, tag) => {
          ...
          if (target?.graph?.edges) {
            ...
            for (let tag2 in found) {
              ...
              const res = await toRun.run(...args2);
              results.children[tag2] = "default" in res ? res.default : res;
              ...
              if (node.plugins) {
                for (let key in node.plugins[nestedName]) {
                  ...
                  if (typeof newInfo === "object") {
                    ...
                    console.error("[wasl-load] Plugin info is not an object...");
                }
              }
            } else {
              onError({
