        ...
        (function(factory) {
          ...
          function FakeBlobBuilder() {
            ...
            function File2(chunks, name2, opts) {
              opts = opts || {};
              var a3 = Blob4.call(this, chunks, opts) || this;
...
${text}`;
  ...
  var merge = (main, override, deleteSrc = false) => {
    ...
    if (override) {
      ...
      keys.forEach((k3) => {
        newKeys.delete(k3);
        if (typeof override[k3] === "object" && !Array.isArray(override[k3]))
  ...
  var get3 = (path2, rel = "", keepRelativeImports = false) => {
    ...
    const splitPath = path2.split("/");
    const pathTokens = splitPath.filter((str, i3) => {
      if (splitPath[i3 - 1] && regex2.test(splitPath[i3 - 1]))
        return true;
      else
        return !!str;
    });
  ...
  var importFromText2 = async (text, path2) => {
    const extension = path2.split(".").slice(-1)[0];
    const isJSON = extension === "json";
    let mimeType = isJSON ? "application/json" : "application/javascript";
    ...
    constructor() {
      ...
      this.subscribeTrigger = (key, onchange) => {
        if (key) {
          ...
          let l3 = this.triggers[key].length;
          this.triggers[key].push({ idx: l3, onchange });
  ...
  function addLocalState(props) {
    if (!this._state)
      this._state = {};
    for (let k3 in props) {
      if (k3 === "_state" || k3 === "graph")
        continue;
      else {
        this._state[k3] = props[k3];
        if (k3 in this)
          this[k3] = props[k3];
        else
          Object.defineProperty(this, k3, {
            get: () => {
              this._state[k3];
            },
            set: (v3) => {
    ...
    constructor(properties = {}, parentNode, graph) {
      ...
      this.setOperator = (operator2) => {
        if (typeof operator2 !== "function")
          return operator2;
        this.operator = operator2.bind(this);
        return operator2;
      };
      this.runAsync = (...args) => {
      ...
      this.runParent = async (n3, ...args) => {
        if (n3.backward && n3.parent) {
          if (typeof n3.parent === "string") {
            if (n3.graph && n3.graph?.get(n3.parent)) {
              n3.parent = n3.graph;
              if (n3.parent)
                this.nodes.set(n3.parent.tag, n3.parent);
            } else
              n3.parent = this.nodes.get(n3.parent);
          }
          if (n3.parent instanceof GraphNode)
            await n3.parent.run(...args);
        }
      };
      this.runChildren = async (n3, ...args) => {
        if (typeof n3.children === "object") {
          for (const key in n3.children) {
            if (typeof n3.children[key] === "string") {
              if (n3.graph && n3.graph?.get(n3.children[key])) {
                n3.children[key] = n3.graph.get(n3.children[key]);
                if (!n3.nodes.get(n3.children[key].tag))
                  n3.nodes.set(n3.children[key].tag, n3.children[key]);
              }
              if (!n3.children[key] && n3.nodes.get(n3.children[key]))
                n3.children[key] = n3.nodes.get(n3.children[key]);
            } else if (typeof n3.children[key] === "undefined" || n3.children[key] === true) {
              if (n3.graph && n3.graph?.get(key)) {
                n3.children[key] = n3.graph.get(key);
                if (!n3.nodes.get(n3.children[key].tag))
                  n3.nodes.set(n3.children[key].tag, n3.children[key]);
              }
              if (!n3.children[key] && n3.nodes.get(key))
                n3.children[key] = n3.nodes.get(key);
            }
            if (n3.children[key]?.runOp)
              await n3.children[key].run(...args);
          }
        }
      };
      this.runBranch = async (n3, output) => {
        if (n3.branch) {
          let keys = Object.keys(n3.branch);
          await Promise.all(keys.map(async (k3) => {
            if (typeof n3.branch[k3].if === "object")
              n3.branch[k3].if = stringifyFast(n3.branch[k3].if);
            let pass = false;
            if (typeof n3.branch[k3].if === "function") {
              pass = n3.branch[k3].if(output);
            } else {
              if (typeof output === "object") {
                if (stringifyFast(output) === n3.branch[k3].if)
                  pass = true;
              } else if (output === n3.branch[k3].if)
                pass = true;
            }
            if (pass) {
              if (n3.branch[k3].then.run) {
                if (Array.isArray(output))
                  await n3.branch[k3].then.run(...output);
                else
                  await n3.branch[k3].then.run(...output);
              } else if (typeof n3.branch[k3].then === "function") {
                if (Array.isArray(output))
                  await n3.branch[k3].then(...output);
                else
                  await n3.branch[k3].then(output);
              } else if (typeof n3.branch[k3].then === "string") {
                if (n3.graph)
                  n3.branch[k3].then = n3.graph.nodes.get(n3.branch[k3].then);
                else
                  n3.branch[k3].then = n3.nodes.get(n3.branch[k3].then);
                if (n3.branch[k3].then.run) {
                  if (Array.isArray(output))
                    await n3.branch[k3].then.run(...output);
                  else
      ...
      this.add = (n3 = {}) => {
        if (typeof n3 === "function")
          n3 = { operator: n3 };
        if (n3?.node instanceof GraphNode)
          n3 = n3.node;
        if (!(n3 instanceof GraphNode))
          n3 = new GraphNode(n3.node ?? n3, this, this.graph);
        this.nodes.set(n3.tag, n3);
        if (this.graph) {
          this.graph.nodes.set(n3.tag, n3);
      ...
      this.remove = (n3) => {
        if (typeof n3 === "string")
          n3 = this.nodes.get(n3);
        if (n3?.tag) {
          this.nodes.delete(n3.tag);
          if (this.children[n3.tag])
            delete this.children[n3.tag];
          if (this.graph) {
            this.graph.nodes.delete(n3.tag);
            this.graph.nNodes = this.graph.nodes.size;
          }
          this.nodes.forEach((n22) => {
            if (n22.nodes.get(n22.tag)) {
              n22.nodes.delete(n22.tag);
              if (n22.children[n22.tag])
                delete n22.children[n22.tag];
              if (n22.parent?.tag === n22.tag)
                delete n22.parent;
            }
          });
          if (n3.ondelete)
      ...
      this.append = (n3, parentNode2 = this) => {
        if (typeof n3 === "string")
          n3 = this.nodes.get(n3);
        if (n3?.nodes) {
          parentNode2.addChildren(n3);
          if (n3.forward)
            n3.runSync = false;
        }
      };
      ...
      this.getProps = (n3 = this, getInitial = true) => {
        let baseprops = {
          tag: n3.tag,
          operator: n3.operator,
          graph: n3.graph,
          ...
          frame: n3.frame,
          delay: n3.delay,
          recursive: n3.recursive,
          repeat: n3.repeat,
          branch: n3.branch,
          oncreate: n3.oncreate,
          reactive: n3.reactive,
          DEBUGNODE: n3.DEBUGNODE
        };
        if (!getInitial) {
          let uniqueprops = {};
      ...
      this.removeTree = (n3) => {
        ...
        if (n3?.nodes) {
            ...
            if (typeof node.children === "object" && !checked[node.tag]) {
              ...
              for (const key in node.children) {
                ...
                if (node.children[key].tag) {
                  ...
                  this.nodes.forEach((n22) => {
                    if (n22.nodes.get(node.children[key].tag))
                      n22.nodes.delete(node.children[key].tag);
                    if (n22.children[key] instanceof GraphNode)
                      delete n22.children[key];
                  });
          ...
          if (n3.tag) {
            ...
            if (this.children[n3.tag])
              delete this.children[n3.tag];
            if (this.parent?.tag === n3.tag)
              delete this.parent;
            if (this[n3.tag] instanceof GraphNode)
              delete this[n3.tag];
            this.nodes.forEach((n22) => {
              if (n22?.tag) {
                if (n22.nodes.get(n22.tag))
                  n22.nodes.delete(n22.tag);
                if (n22.children[n22.tag] instanceof GraphNode)
                  delete n22.children[n22.tag];
              }
            });
            recursivelyRemove(n3);
            if (this.graph)
              this.graph.removeTree(n3, checked);
            else if (n3.ondelete)
              n3.ondelete(n3);
          }
        }
      };
      this.checkNodesHaveChildMapped = (n3, child, checked = {}) => {
        let tag = n3.tag;
        if (!tag)
          tag = n3.name;
        if (!checked[tag]) {
          checked[tag] = true;
          if (n3.children) {
            if (child.tag in n3.children) {
              if (n3.children[child.tag] instanceof GraphNode) {
                if (!n3.nodes.get(child.tag))
                  n3.nodes.set(child.tag, child);
                n3.children[child.tag] = child;
                if (!n3.firstRun)
                  n3.firstRun = true;
              }
            }
          ...
          if (n3.parent instanceof GraphNode) {
            if (n3.nodes.get(child.tag))
              n3.parent.nodes.set(child.tag, child);
            if (n3.parent.children) {
              this.checkNodesHaveChildMapped(n3.parent, child, checked);
            } else if (n3.nodes) {
              n3.nodes.forEach((n22) => {
                if (!checked[n22.tag]) {
                  this.checkNodesHaveChildMapped(n22, child, checked);
                }
              });
            }
          }
          if (n3.graph) {
            if (n3.parent && n3.parent.name !== n3.graph.name) {
              n3.graph.nodes.forEach((n22) => {
                if (!checked[n22.tag]) {
                  this.checkNodesHaveChildMapped(n22, child, checked);
                }
              });
      ...
      this.convertChildrenToNodes = (n3 = this) => {
        if (n3?.children) {
          for (const key in n3.children) {
            if (!(n3.children[key] instanceof GraphNode)) {
              if (typeof n3.children[key] === "object") {
                if (!n3.children[key].tag)
                  n3.children[key].tag = key;
                if (!n3.nodes.get(n3.children[key].tag)) {
                  n3.children[key] = new GraphNode(n3.children[key], n3, n3.graph);
                  this.checkNodesHaveChildMapped(n3, n3.children[key]);
                }
              } else {
                if (typeof n3.children[key] === "undefined" || n3.children[key] == true) {
                  n3.children[key] = n3.graph.get(key);
                  if (!n3.children[key])
                    n3.children[key] = n3.nodes.get(key);
                } else if (typeof n3.children[key] === "string") {
                  let k3 = n3.children[key];
                  n3.children[key] = n3.graph.get(k3);
                  if (!n3.children[key])
                    n3.children[key] = n3.nodes.get(key);
                }
                if (n3.children[key] instanceof GraphNode) {
                  n3.nodes.set(n3.children[key].tag, n3.children[key]);
                  this.checkNodesHaveChildMapped(n3, n3.children[key]);
                  if (!(n3.children[key].tag in n3))
                    n3[n3.children[key].tag] = n3.children[key];
                }
              }
            }
          }
      ...
      this.stopLooping = (n3 = this) => {
        n3.isLooping = false;
      };
      this.stopAnimating = (n3 = this) => {
        n3.isAnimating = false;
      };
      this.stopNode = (n3 = this) => {
        n3.stopAnimating(n3);
        n3.stopLooping(n3);
      };
      this.subscribeNode = (n3) => {
        if (typeof n3 === "string")
          n3 = this.nodes.get(n3);
        if (n3.tag)
          this.nodes.set(n3.tag, n3);
      ...
      this.print = (n3 = this, printChildren = true, nodesPrinted = []) => {
        let dummyNode = new GraphNode();
        if (typeof n3 === "string")
          n3 = this.nodes.get(n3);
        if (n3 instanceof GraphNode) {
          nodesPrinted.push(n3.tag);
          let jsonToPrint = {
            tag: n3.tag,
            operator: n3.operator.toString()
          };
          if (n3.parent)
            jsonToPrint.parent = n3.parent.tag;
          if (typeof n3.children === "object") {
            for (const key in n3.children) {
              if (typeof n3.children[key] === "string")
                return n3.children[key];
              if (nodesPrinted.includes(n3.children[key].tag))
                return n3.children[key].tag;
              else if (!printChildren) {
                return n3.children[key].tag;
              } else
                return n3.children[key].print(n3.children[key], printChildren, nodesPrinted);
            }
          }
          for (const prop in n3) {
            if (prop === "parent" || prop === "children")
              continue;
            if (typeof dummyNode[prop] === "undefined") {
              if (typeof n3[prop] === "function") {
                jsonToPrint[prop] = n3[prop].toString();
              } else if (typeof n3[prop] === "object") {
                jsonToPrint[prop] = JSON.stringifyWithCircularRefs(n3[prop]);
              } else {
                jsonToPrint[prop] = n3[prop];
              }
            }
          }
      ...
      if (typeof properties === "object") {
        ...
        if (properties instanceof Graph) {
          ...
          source.node = this;
          if (graph) {
            source.nodes.forEach((n3) => {
              if (!graph.nodes.get(n3.tag)) {
                graph.nodes.set(n3.tag, n3);
                graph.nNodes++;
              }
            });
          }
  ...
  var Graph = class {
    constructor(tree, tag, props) {
      ...
      this.add = (n3 = {}) => {
        ...
        let props2 = n3;
        if (!(n3 instanceof GraphNode))
          n3 = new GraphNode(props2?.node ?? props2, this, this);
        else {
          this.nNodes = this.nodes.size;
          if (n3.tag) {
            this.tree[n3.tag] = props2;
            this.nodes.set(n3.tag, n3);
          }
        }
        return n3;
          ...
          if (!n3) {
            if (typeof tree2[node] === "function") {
              ...
              if (tree2[node].aliases) {
                tree2[node].aliases.forEach((a3) => {
                  this.nodes.set(a3, newNode);
                });
              }
            } else {
            ...
            if (typeof tree2[node] === "function") {
              ...
              if (tree2[node] instanceof GraphNode) {
                ...
                n3.setProps(properties);
              } else {
                n3.setProps(tree2[node]);
              }
            }
          }
        }
      ...
      this.set = (n3) => {
        return this.nodes.set(n3.tag, n3);
      };
      this.run = (n3, ...args) => {
        if (typeof n3 === "string")
          n3 = this.nodes.get(n3);
              ...
              if (Array.isArray(node.children)) {
                node.children.forEach((c3) => {
                  if (c3.stopNode)
                    c3.stopNode();
                  if (c3.tag) {
                    if (this.nodes.get(c3.tag))
                      this.nodes.delete(c3.tag);
                  }
                  this.nodes.forEach((n22) => {
                    if (n22.nodes.get(c3.tag))
                      n22.nodes.delete(c3.tag);
                  });
                  recursivelyRemove(c3);
                });
                ...
                this.nodes.forEach((n22) => {
                  if (n22.nodes.get(node.tag))
                    n22.nodes.delete(node.tag);
                });
      ...
      this.append = (n3, parentNode) => {
        parentNode.addChildren(n3);
      };
      this.callParent = async (n3, ...args) => {
        if (n3?.parent) {
          return await n3.callParent(...args);
        }
      };
      this.callChildren = async (n3, ...args) => {
        if (n3?.children) {
          return await n3.callChildren(...args);
        }
      };
      this.subscribe = (n3, callback) => {
        if (!callback)
          return;
        if (n3?.subscribe && typeof callback === "function") {
          return n3.subscribe(callback);
        } else if (callback instanceof GraphNode || typeof callback === "string")
          return this.subscribeNode(n3, callback);
        else if (typeof n3 == "string") {
          return this.state.subscribeTrigger(n3, callback);
        }
      };
      ...
      this.print = (n3, printChildren = true) => {
        if (n3?.print)
          return n3.print(n3, printChildren);
        else {
          let printed = `{`;
          this.nodes.forEach((n22) => {
            printed += `
"${n22.tag}:${n22.print(n22, printChildren)}"`;
          });
          return printed;
        }
      };
      ...
      this.create = (operator2, parentNode, props2) => {
        return createNode(operator2, parentNode, props2, this);
      };
      this.setState = (data) => {
        this.state.setState(data);
      };
        ...
        if (typeof value === "object") {
          ...
          if (c3 === "Array") {
            if (value.length > 20) {
              ...
              val = value;
          } else if (c3.includes("Set")) {
            val = Array.from(value);
          } else if (c3 !== "Object" && c3 !== "Number" && c3 !== "String" && c3 !== "Boolean") {
            val = "instanceof_" + c3;
          } else if (c3 === "Object") {
            let obj = {};
  ...
  function createNode(operator2, parentNode, props, graph) {
    if (typeof props === "object") {
      props.operator = operator2;
      return new GraphNode(props, parentNode, graph);
    }
    return new GraphNode({ operator: operator2 }, parentNode, graph);
  }
  var DOMElement = class extends HTMLElement {
    ...
    attributeChangedCallback = (name2, old, val) => {
      if (name2 === "onchanged") {
        ...
        if (typeof rpc === "function") {
          this.renderonchanged = this.state.subscribeTrigger("props", (p3) => {
            this.render(p3);
            rpc(this, p3);
          });
        } else if (rpc != false)
    ...
    render = (props = this.props) => {
      ...
      const t9 = document.createElement("template");
      if (typeof this.templateResult === "string")
        t9.innerHTML = this.templateResult;
        ...
        t9.appendChild(this.templateResult);
      }
      const fragment = t9.content;
      if (this.FRAGMENT) {
        if (this.useShadow) {
          if (this.STYLE)
      ...
      if (this.useShadow) {
        ...
        if (this.styles) {
          ...
          style2.textContent = this.styles;
          this.shadowRoot.prepend(style2);
          this.STYLE = style2;
        }
    ...
    state = {
      ...
      subscribeTrigger(key, onchanged = (res) => {
        ...
        if (key) {
          ...
          let l3 = this.triggers[key].length;
          this.triggers[key].push({ idx: l3, onchanged });
          return this.triggers[key].length - 1;
        } else
          return void 0;
      },
  ...
  var Service = class extends Graph {
    constructor(options2 = {}) {
      ...
      this.init = (options3) => {
        ...
        if (Array.isArray(options3.routes)) {
          options3.routes.forEach((r3) => {
            this.load(r3, options3.includeClassName, options3.routeFormat, options3.customRoutes, options3.customChildren, options3.sharedState);
          });
      ...
      this.load = (routes, includeClassName = true, routeFormat = ".", customRoutes, customChildren, sharedState = true) => {
              ...
              let checkChildGraphNodes = (nd, par) => {
                if (!checked[nd.tag] || par && includeClassName && !checked[par?.tag + routeFormat + nd.tag]) {
                  ...
                  if (nd instanceof Graph || nd.source instanceof Graph || nd.setTree) {
                    ...
                    nd.nodes.forEach((n3) => {
                      if (includeClassName && !routes[nd.tag + routeFormat + n3.tag])
                        routes[nd.tag + routeFormat + n3.tag] = n3;
                      else if (!routes[n3.tag])
                        routes[n3.tag] = n3;
                      checkChildGraphNodes(n3, nd);
                    });
                  }
                }
              };
                ...
                nested:
                  for (const key in route.children) {
                    ...
                    if (typeof route.children[key] === "object") {
                      let rt3 = route.children[key];
                      if (rt3.tag && allRoutes[rt3.tag])
                        continue;
                      if (customChildren) {
                        for (const k22 in customChildren) {
                          rt3 = customChildren[k22](rt3, key, route, routes, allRoutes);
                          if (!rt3)
                            continue nested;
                        }
                      }
                      ...
                      let k3;
                      if (rt3.tag) {
                        if (allRoutes[rt3.tag]) {
                          let randkey = `${rt3.tag}${incr}`;
                          allRoutes[randkey] = rt3;
                          rt3.tag = randkey;
                          childrenIter(allRoutes[randkey], key);
                          k3 = randkey;
                        } else {
                          allRoutes[rt3.tag] = rt3;
                          childrenIter(allRoutes[rt3.tag], key);
                          k3 = rt3.tag;
                        }
                      } else {
                        if (allRoutes[key]) {
                      ...
                      if (service?.name && includeClassName) {
                        allRoutes[service.name + routeFormat + k3] = rt3;
                        delete allRoutes[k3];
                      } else
                        allRoutes[k3] = rt3;
                    }
        ...
        for (const prop in this.routes) {
          if (this.routes[prop]?.aliases) {
            ...
            aliases.forEach((a3) => {
              if (service?.name && includeClassName)
                routes[service.name + routeFormat + a3] = this.routes[prop];
              else
                routes[a3] = this.routes[prop];
            });
          }
        }
        return this.routes;
    ...
    spliceTypedArray(arr, start, end) {
      ...
      if (s3.length > 0 || e3?.length > 0)
        n3 = new arr.constructor(s3.length + e3.length);
      if (s3.length > 0)
        n3.set(s3);
      if (e3 && e3.length > 0)
        n3.set(e3, s3.length);
  ...
  var DOMService = class extends Service {
    constructor(options2, parentNode, interpreters) {
      ...
      this.customChildren = {
        "dom": (rt3, routeKey, route, routes, checked) => {
          if ((route.tag || route.id) && (route.template || route.context || route.tagName || route.element) && (rt3.template || rt3.context || rt3.tagName || rt3.element) && !rt3.parentNode) {
            if (route.tag)
              rt3.parentNode = route.tag;
            if (route.id)
              rt3.parentNode = route.id;
          }
          return rt3;
        }
      };
      ...
      this.addElement = (options3, generateChildElementNodes = false) => {
        ...
        if (generateChildElementNodes) {
          divs = divs.map((d3, i3) => this.addElement({ element: d3 }));
        }
      ...
      this.addComponent = (options3, generateChildElementNodes = true) => {
        ...
        let divs = Array.from(elm.querySelectorAll("*"));
        if (generateChildElementNodes) {
          divs = divs.map((d3) => this.addElement({ element: d3 }));
        }
        if (!options3.element)
          options3.element = elm;
        if (!options3.operator)
      ...
      this.terminate = (element) => {
        if (typeof element === "object") {
          ...
          if (this.components[element].divs)
            this.components[element].divs.forEach((d3) => this.terminate(d3));
          ...
          if (this.elements[element].divs)
            this.elements[element].divs.forEach((d3) => this.terminate(d3));
  ...
  var Router = class extends Service {
    constructor(options2) {
      ...
      this.addUser = async (info, connections, config, receiving) => {
            ...
            if (typeof connections[key] === "object") {
              if (!connections[key].connection._id) {
                await new Promise((res, rej) => {
                  ...
                  console.error("Connections timed out:", er2);
                });
              }
            }
          }
        ...
        if (config) {
          for (const c3 in config) {
            this.openConnection(config[c3].service, config[c3], user._id, config[c3].args);
          }
        }
        let send = (message, ...a3) => {
          let connection = this.getConnection(user._id, "send");
          if (connection?.send)
            return connection.send(message, ...a3);
        };
        let request = (message, method, ...a3) => {
          let connection = this.getConnection(user._id, "request");
          if (connection?.request)
            return connection.request(message, method, ...a3);
        };
        let post = (route, args, method, ...a3) => {
          let connection = this.getConnection(user._id, "post");
          if (connection?.post)
            return connection.post(route, args, method, ...a3);
        };
        let run = (route, args, method, ...a3) => {
          let connection = this.getConnection(user._id, "run");
          if (connection?.run)
            return connection.run(route, args, method, ...a3);
        };
        let subscribe = (route, callback, ...a3) => {
          let connection = this.getConnection(user._id, "subscribe");
          if (connection?.subscribe)
            return connection.subscribe(route, callback, ...a3);
        };
        let unsubscribe = (route, sub, ...a3) => {
          let connection = this.getConnection(user._id, "unsubscribe");
          if (connection?.unsubscribe)
            return connection.unsubscribe(route, sub, ...a3);
        };
        let terminate = () => {
        ...
        if (connections && !receiving) {
          let connectionIds = {};
          let pass = false;
          Object.keys(connections).map((k3, i3) => {
            if (connections[k3]?._id) {
              connectionIds[`${i3}`] = connections[k3]?._id;
              pass = true;
            }
          });
          if (pass) {
      ...
      this.getConnection = (sourceId, hasMethod) => {
        if (this.sources[sourceId]) {
          if (this.order) {
              ...
              for (const key in this.sources[sourceId]) {
                if (this.sources[sourceId][key].service) {
                  if (typeof this.sources[sourceId][key].service === "object") {
                    if (this.sources[sourceId][key].service.tag === k3) {
                  ...
                  } else if (this.sources[sourceId][key].service === k3) {
                    if (this.sources[sourceId][key].connectionType && this.sources[sourceId][key].service?.name) {
                      if (!this.serviceConnections[this.sources[sourceId][key].service.name])
                        this.removeConnection(this.sources[sourceId][key]);
                      continue;
            ...
            for (const k3 in this.sources[sourceId]) {
              if (this.sources[sourceId][k3].connectionType && this.sources[sourceId][k3].service?.name) {
                if (!this.serviceConnections[this.sources[sourceId][k3].service.name]) {
                  this.removeConnection(this.sources[sourceId][k3]);
                  continue;
                }
              }
              if (hasMethod && this.sources[sourceId][k3][hasMethod]) {
                return this.sources[sourceId][k3];
              } else {
                return this.sources[sourceId][k3];
              }
            }
          ...
          for (let i3 = 0; i3 < this.order.length; i3++) {
            let k3 = this.order[i3];
            if (this.sources[k3]?.[sourceId]) {
              if (this.sources[k3][sourceId].connectionType && this.sources[k3][sourceId].service?.name) {
                if (!this.serviceConnections[this.sources[k3][sourceId].service.service.name]) {
                  this.removeConnection(this.sources[k3][sourceId].service);
                  continue;
                }
              }
              if (hasMethod && this.sources[k3][sourceId]?.[hasMethod]) {
                return this.sources[k3][sourceId];
              } else {
                return this.sources[k3][sourceId];
              }
            }
          ...
          for (const key in this.sources[sourceId]) {
            if (typeof this.sources[sourceId][key] === "object") {
              if (!this.sources[sourceId][key]._id) {
                for (const k3 in this.sources[sourceId][key]) {
                  if (typeof this.sources[sourceId][key][k3] === "object") {
                    let pass = true;
                    if (hasMethod && !this.sources[sourceId][key][k3][hasMethod])
                      pass = false;
                    for (const p3 in props) {
                      if (typeof this.sources[sourceId][key][k3][p3] === "object" && typeof props[p3] === "object") {
                        for (const pp in props[p3]) {
                          if (props[p3][pp] !== this.sources[sourceId][key][k3][p3][pp]) {
                            pass = false;
                            break;
                          }
                        }
                      } else if (this.sources[sourceId][key][k3][p3] !== props[p3]) {
                        pass = false;
                      } else {
                        pass = false;
                        break;
                    ...
                    if (pass) {
                      found[this.sources[sourceId][key][k3]._id] = this.sources[sourceId][key][k3];
                    }
                  }
                }
              } else {
                ...
                if (hasMethod && !this.sources[sourceId][key][hasMethod])
                  pass = false;
                for (const p3 in props) {
                  if (typeof this.sources[sourceId][key][p3] === "object" && typeof props[p3] === "object") {
                    for (const pp in props[p3]) {
                      if (props[p3][pp] !== this.sources[sourceId][key][p3][pp]) {
                        pass = false;
                        break;
                      }
                    }
                  } else if (this.sources[sourceId][key][p3] !== props[p3]) {
                    pass = false;
                  } else {
                    pass = false;
                    break;
      ...
      this.addConnection = (options3, source) => {
        ...
        if (typeof options3 === "string") {
          if (this.connections[options3]) {
            ...
            for (const j3 in this.serviceConnections) {
              for (const k3 in this.serviceConnections[j3]) {
                if (this.serviceConnections[j3][k3][options3]) {
                  options3 = { connection: this.serviceConnections[j3][k3][options3] };
                  options3.service = j3;
                  settings.connectionType = j3;
                  settings.connectionsKey = k3;
                  break;
                }
              }
            }
        ...
        if (options3.connection instanceof GraphNode) {
          ...
          settings.post = async (route, args, method) => {
            if (route && node.get(route)) {
              let n3 = node.get(route);
              if (method) {
                if (Array.isArray(args)) {
                  return n3[method]?.(...args);
                } else
                  return n3[method]?.(args);
              } else {
                if (Array.isArray(args)) {
          ...
          settings.terminate = (n3) => {
            graph.remove(n3);
            return true;
          };
        } else if (!(options3._id && this.connections[options3._id])) {
          let c3 = options3.connection;
          if (typeof c3 === "string") {
            if (this.connections[c3])
              c3 = this.connections[c3];
              ...
              if (typeof options3.service === "object") {
                if (options3.service.connections) {
                  for (const key in options3.service.connections) {
                    if (options3.service.connections[key][c3]) {
                      c3 = options3.service.connections[key][c3];
                      settings.connectionType = key;
                      settings.connectionsKey = c3;
                      break;
                    }
                  }
                }
              ...
              for (const j3 in this.serviceConnections) {
                for (const k3 in this.serviceConnections[j3]) {
                  if (this.serviceConnections[j3][k3][c3]) {
                    c3 = this.serviceConnections[j3][k3][c3];
                    options3.service = j3;
                    settings.connectionType = j3;
                    settings.connectionsKey = k3;
                    break;
          ...
          if (typeof c3 !== "object")
            return void 0;
          settings._id = c3._id;
          settings.send = c3.send;
          settings.request = c3.request;
          settings.run = c3.run;
          settings.post = c3.post;
          settings.subscribe = c3.subscribe;
          settings.unsubscribe = c3.unsubscribe;
          settings.terminate = c3.terminate;
          settings.onclose = options3.onclose;
          if (settings.onclose) {
            if (!(c3.onclose && settings.onclose.toString() === c3.onclose.toString())) {
              let oldonclose = c3.onclose;
              c3.onclose = (...args) => {
                if (settings.onclose)
          ...
          if (this.connections[connection]) {
            ...
            for (const key in this.sources) {
              if (this.sources[key][connection])
                ...
                for (const k3 in this.sources[key]) {
                  if (this.sources[key][k3]?.[connection]) {
                    delete this.sources[key][connection];
                  }
                }
              }
      ...
      this.addService = (service, connections, includeClassName, routeFormat, syncServices, source, order) => {
        ...
        if (connections) {
          if (typeof connections === "string")
            ...
            for (const c3 in connections) {
              this.addServiceConnections(service, c3, source);
            }
          }
        }
        if (syncServices)
            ...
            transmitter.subscribe(route, transmitter._id, (res3) => {
              if (!this.connections[receiver._id] && rxsrc) {
                if (this.sources[rxsrc]) {
                  rxsrc = receiver;
                  Object.keys(this.sources[rxsrc]).forEach((k3) => {
                    if (this.sources[receiver][k3].send) {
                      receiver = this.sources[receiver][k3];
                    }
                  });
                }
              }
      ...
      this.syncServices = () => {
        for (const name2 in this.services) {
          ...
          this.nodes.forEach((n3, tag) => {
            if (!this.services[name2].nodes.get(n3.tag)) {
              this.services[name2].nodes.set(n3.tag, n3);
            } else {
              if (!this.services[name2].nodes.get(tag) && n3._UNIQUE !== this.services[name2].nodes.get(n3.tag)._UNIQUE)
                this.services[name2].nodes.set(tag, n3);
            }
          });
        }
      };
              ...
              if (typeof opt.service === "object") {
                if (opt.connections) {
                  if (Array.isArray(opt.connections)) {
                    opt.connections.forEach((k3) => {
                      this.addServiceConnections(opt[key].service, k3);
                    });
                  } else
                    this.addServiceConnections(opt.service, opt.connections);
                }
                if (opt.config) {
                  for (const c3 in opt.config) {
                    this.openConnection(opt.service, opt.config[c3], opt.config[c3].source, opt.config[c3].args);
                  }
                }
              }
            }
  ...
  var ESPlugin = class {
    ...
    constructor(node, options2 = {}) {
      ...
      if (hasGraph) {
        ...
        toNotify.forEach((o3) => {
          let tag = o3.#options.tag;
          if (thisTag)
            tag = `${thisTag}.${tag}`;
          this.plugins[o3.#options.tag] = o3;
          if (typeof options2.onPlugin === "function")
            options2.onPlugin(tag, o3);
        });
      }
    ...
    stop = () => {
      for (let k3 in this.nested)
        this.nested[k3].stop();
      if (this.graph)
        this.graph.nodes.forEach((n3) => {
          this.graph.removeTree(n3);
          n3.stopNode();
      ...
      if (!("default" in info) || info instanceof Graph)
        ...
        if (info.arguments) {
          ...
          for (let key in info.arguments) {
            const v3 = info.arguments[key];
            if (isArray) {
              argsArray[i3].state = v3;
              if (i3 == 0)
                this.#toRun = true;
            } else {
              args.get(key).state = v3;
              if (input === key)
                this.#toRun = true;
            }
            i3++;
    ...
    constructor(urlOrObject, options2 = {}, url) {
      ...
      this.init = async (urlOrObject = __privateGet(this, _input), options2 = __privateGet(this, _options), url = "", symbols = [], counter = 0) => {
        ...
        if (typeof urlOrObject === "object") {
          ...
          if (!url)
            url = resolve2(urlOrObject, options2.relativeTo ?? "");
          mode = "import";
        } else
        ...
        if (this.errors.length === 0) {
          ...
          const drillToTest = (target) => {
            drill(target, (node, info) => {
              ...
              for (let output in edges) {
                const getTarget = (o3, str) => o3.graph?.nodes?.[str] ?? o3[str];
            ...
            if (clonedOptions.output !== "object") {
              ...
              let drillCopy = (target) => {
                if (target?.graph) {
                  ...
                  if (nodes2) {
                    for (let k3 in nodes2) {
                      nodes2[k3] = Object.assign({}, nodes2[k3].initial);
                      drillCopy(nodes2[k3]);
                    }
                  }
                  target.graph = graph;
  ...
  var validate = async (urlOrObject, options2 = {}, load = true) => {
    ...
    if (errors.length === 0) {
      ...
      if (load && typeof options2.wasl === "function") {
          ...
          graph: {
            nodes: {
              button_1: {
                ...
                src: "../../plugins/ui/button/index.js"
              },
              button_2: {
                attributes: {
              ...
              display: {
                src: "../../plugins/ui/display/index.js"
              }
            }
          }
        }
  ...
  // ../htil/plugins/ui/button/index.js
  var button_exports = {};
  ...
  // ../htil/plugins/ui/display/index.js
  var display_exports = {};
  ...
  // ../htil/plugins/ui/container/index.js
  ...
  var safeParse = (input) => {
    ...
    if (typeof input === "object") {
      for (let key in input) {
        let value = input[key];
        let regex3 = new RegExp("(|[a-zA-Z]w*|([a-zA-Z]w*(,s*[a-zA-Z]w*)*))s*=>");
        let func = typeof value === "string" ? value.substring(0, 8) == "function" : false;
        let arrow = typeof value === "string" ? regex3.test(value) : false;
        try {
  ...
  var options = {
    ...
    filesystem: {
      ...
      "plugins/ui/button/index.js": button_exports,
      "plugins/ui/display/index.js": display_exports,
      "plugins/ui/container/index.js": container_exports,
  ...
  var startExecution = async () => {
    ...
    for (let i3 in info) {
      let o3 = info[i3];
      if (o3.wasl) {
        console.log(`------------------ ${o3.name.toUpperCase()} MODE ------------------`);
        await o3.wasl.init();
        await o3.wasl.start();
        const str = JSON.stringify(o3.wasl.original, null, 2);
        o3.div.value = str;
        strArr.push(str);
        refArr.push(o3.wasl.original);
        flowArr.push(o3.wasl.flow);
        resArr.push(o3.wasl.resolutions);
      } else
        o3.div.value = void 0;
    }
